...
\end{tcolorbox}
$$ $$
$\bullet$ Let's remind what a well ordered set means:\\
If $(S,\leq )$ is a partially ordered set such that every nonempty subset of $S$ has a first element, then $\neq$ is said to be a well ordering for $S$ and $(S,\neq )$ is said to be a well-ordered set.\\
$\bullet$ And a maximal element $m$ of a partially ordered set $(S,\leq )$ implies that $\forall s\in S: m\leq s\implies m=s$.\\
$\bullet$ A linearly ordered set means that $\forall x,y\in S: x\leq y\text{ or } y\leq x$. \\\\
