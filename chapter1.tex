...
First $S$ is a (possibly infinite) set of finite sequences $f:\Pee_n\rightarrow X$. As $X$ could be an infinite set and the $f$ are finite sequences, $S$ can be an infinite set. Considering the elements (sequences) $f\in S$ as sets we can state that 
$$ \bigcup S= X$$
What is this map $G$?\\
$G$ will take an element (sequence) of $S$ and map it to one element of $X$. So if $X= \Qiuu$, a sequence $f$ will be mapped to a rational number.}\\\\
$$\lozenge$$
