...
\end{tcolorbox}
Suppose $p$ is a limit point of $S^{'}$ but not an element of $S^{'}$. This means
$$\forall \epsilon>0: \left(N(p;\epsilon)-{p}\right)\cap S^{'}\neq \emptyset$$
As $p\not \in  S^{'}$ we get $\forall \epsilon>0: N(p;\epsilon)\cap S^{'}\neq \emptyset$. So there is at least a $y\in N(p;\epsilon)$ which is also in $S^{'}$. By definition of $S^{'}$, $y$ is a limit point of $S$. This means
$$\forall \epsilon>0: \left(N(y;\epsilon)-{y}\right)\cap S^{}\neq \emptyset$$
This means that $\exists x\in S: x\in N(y;\epsilon)$ or $d(x,y) <\epsilon$. Also, as $y\in N(p;\epsilon)$ we have $d(p,y) < \epsilon$.
We have (triangle inequality)
$$d(p,x)\leq d(p,y)+d(y,x)$$
As $d(x,y) <\epsilon$ and $d(p,y) < \epsilon$ we get 
$$d(p,x)<\epsilon^{'} \quad \text{(with }\quad \epsilon^{'}= \frac{\epsilon}{2}\text{)}$$
Hence $p\in N(x;\epsilon^{'})$
$$\blacklozenge$$\\
...
\sim S\right)  =N(p;\epsilon)-\{p\}(\neq \emptyset)$. Also, no other element $q$ of $\sim S$ can comply with the condition $\forall \epsilon > 0, \, N(q; \epsilon)\cap S \neq \emptyset$ and hence $F=S$. So $F$ has no limit point (and thus does not contain any), hence $F$ is not closed.
$$\blacklozenge$$\\
...
First note that by theorem $\mathbf{39.3 (c)}$ (\textit{The
union of a finite collection of closed sets is closed}) for polygons, it suffice to prove that a line is a closed subset as a polygon is the union of a finite number of lines.
$$\blacklozenge$$\\
\newpage
