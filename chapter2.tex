...
\end{tcolorbox}
Consider $\sim F$. Then, all $p\in F$ are limit points of $\sim F$. Indeed, we have, $\forall \epsilon>0: \left(N(p;\epsilon)-\{p\}\right)\subset \sim F$ and thus $\forall \epsilon>0: \left(N(p;\epsilon)-\{p\}\right)\cap \sim F\neq \emptyset$.\\ Conclusion $p$ is a limit point of $\sim F$. \\
And as $p\not\in \sim F$ we can rewrite this as $\forall \epsilon>0: N(p;\epsilon)\cap \sim F\neq \emptyset$. Also, consider an arbitrary point $z\in\sim F$. Be $E=\{|p_1-z|,|p_2-z|,,\dots, |p_n-z|\}$ where the $p_i$ are the elements of $F$. Be $\delta = \min E$ and choose $\epsilon^{'} < \delta$, the we will have that $N(z;\epsilon^{'})\subset \sim F$ and thus $\sim F$ is an open subset and by theorem $\mathbf{39.2}$ $F$ is a closed subset.
$$\blacklozenge$$\\
...
\end{align*}
 We can not conclude without further arguments that $N(a;\epsilon)-\{a\}\neq \emptyset$. Indeed consider the case where $a_{n_i}=a,\, n_i\geq N$ . But this case is trivial as $a_{n_i}=a,\, n_i\geq N$ is an element of $S$ and thus $a\in S$ (in this case, the requirement  $S$ is closed  is not necessary).\\\\
...
\end{tcolorbox}
First note that if $X=\emptyset$ then the theorem is true as $\emptyset$ is a subset of any set.\\
...
So, be $\mathscr{B}$ an infinite, non-countable, collection of open subsets $B\in \mathscr{B}$. Be $B^{*}$ an arbitrary element of $\mathscr{B}$. $B^{*}$ has at least one point in $X$ and by theorem $\mathbf{43.1}$ we know that we can find a point $q$ (with all rational numbers as coordinates), which is also a point of $B^{*}$. As $q\in D$ with $D$ countable, there is a bijection $f:\Pee\rightarrow D$  and can find a unique $i\in\Pee$ such that $f(i)=q$ and can label the set $B^{*}$ and put $B^{*}_i= B^{*}$. Consider now $X_i= X- B^{*}_i$. 
\begin{figure}[H]%
    \centering
\centering
    \subfloat[ ]{\input{./images/fig_2.43.1.3_a.tex}}
    \subfloat[$X_i= X-B^{*}_i$ ]{\input{./images/fig_2.43.1.3_b.tex}}\\
    \subfloat[$X_j= X_i-(B^{*}_j\cup B^{*}_j)$ ]{\input{./images/fig_2.43.1.3_c.tex}}
        \subfloat[$X_k= X_i-(B^{*}_j\cup B^{*}_j\cup B^{*}_k) $ ]{\input{./images/fig_2.43.1.3_d.tex}}
\caption{Lindelöf Theorem for $\Breal^n$}
\label{fig:fig_p8a}
\end{figure}
Choose an arbitrary point $p^{'}\in X_i$ and a $B^{**}$ such that $p^{'}\in B^{**}$. Consider the subset $ B^{**}-B^{*}_i$. As previously seen (notice that $p^{'}\not \in B^{*}_i$), this subset will contain an element $q_j\in D$ and can label the set $B^{**}$ and put $B^{*}_j= B^{**}$. The next step is than to consider $X_j= X_i-(B^{*}_i\cup B^{*}_j)$ and repeat the procedure i.e. choose an arbitrary point $p^{"}$ and a $B^{***}\in\mathscr{B}$ such that $p^{"}\in B^{***}$. Consider the set $B^{***}- (B^{*}_i\cup B^{*}_j)$. There will be a $q_k\in D \Et \in B^{***}- (B^{*}_i\cup B^{*}_j)$ and we label $B^{*}_k= B^{***}$. Doing that recursively (see figure as illustration), as at each step $X$ is reduced in elements, and knowing that every point in $X$ is covered by a open subset $B\in\mathscr{B}$, we will be able to "cover" $X$ with open subsets $B^{*}_m\in \mathscr{B}$ but for each of these subsets we can map a $m\in \Pee$ such that $\bigcup \mathscr{B^{*}}$ covers $X$ and hence

$$\mathscr{B^{*}}\text{ is countable and } X\subset \bigcup \mathscr{B^{*}}$$
$$\lozenge$$
\newpage
\begin{tcolorbox}[colback=white]
$\mathbf{43.4.}$ \textbf{Heine-Borel Theorem}. Suppose that  $X$ is a bounded and closed subset of $\Breal^n$. Let $\mathscr{K} $ be  a collection of open subsets of $\Breal^n$ that covers $X$. Then some finite subscollection of  $\mathscr{K}$ also covers $X$.
\end{tcolorbox}
...
\begin{tcolorbox}
Suppose that $\{K_i:i\in\Pee\}$ is a collection of nonempty bounded and closed subsets of $\Breal_n$ such that $K_{i+1}\subset K_i$, for $i\in \Pee$. Is $\bigcap\{K_i:i\in\Pee\}$ necessarily nonempty?
\end{tcolorbox}
